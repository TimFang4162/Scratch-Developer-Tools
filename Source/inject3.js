console.log("Scratch Developer Tools Extension");
...
function initGUI() {
  ...
  function getScratchBlocks() {
    ...
    for (const root of b) {
      if (root.type === "event_whenbroadcastreceived") {
        addBlock("event", "当接收到(广播)", root);
        continue;
      }
      if (root.type === "event_broadcast") {
        addBlock("event", "广播(消息)", root);
        continue;
      }
      if (root.type === "event_broadcastandwait") {
        addBlock("event", "广播(消息)并等待", root);
        continue;
      }
      if (root.type === "stringExt_charCodeAt") {
        addBlock("stringExt", "str.(()的第()个字符的编码)", root);
        continue;
      }
      if (root.type === "stringExt_fromCharCode") {
        addBlock("stringExt", "str.(编码()对应的字符)", root);
        continue;
      }
      if (root.type === "stringExt_serializeToJson") {
        addBlock("stringExt", "str.(将()开头的变量转换为JSON)", root);
        continue;
      }
      if (root.type === "stringExt_deserializeFromJson") {
        addBlock("stringExt", "str.将()开头的变量设为JSON", root);
        continue;
      }
      if (root.type === "stringExt_postJson") {
        addBlock("stringExt", "str.发送JSON()到()", root);
        continue;
      }
      if (root.type === "stringExt_postResponse") {
        addBlock("stringExt", "str.(发送JSON应答)", root);
        continue;
      }
      if (root.type === "js_serializeToJson") {
        addBlock("js", "js.(将()开头的变量转换为JSON)", root);
        continue;
      }
      if (root.type === "js_deserializeFromJson") {
        addBlock("js", "js.将()开头的变量设为JSON()", root);
        continue;
      }
      if (root.type === "js_postJson") {
        addBlock("js", "js.发送JSON()到()", root);
        continue;
      }
      if (root.type === "js_postResponse") {
        addBlock("js", "js.(发送JSON应答)", root);
        continue;
      }
      if (root.type === "js_callWorker") {
        addBlock("js", "js.(callWorker)", root);
        continue;
      }
      ...
      if (root.type === "canvas_beginPath") {
        addBlock("canvas", "beginPath", root);
        continue;
      }
      if (root.type === "canvas_closePath") {
        addBlock("canvas", "closePath", root);
        continue;
      }
      if (root.type === "canvas_moveTo") {
        addBlock("canvas", "moveTo", root);
        continue;
      }
      if (root.type === "canvas_lineTo") {
        addBlock("canvas", "lineTo", root);
        continue;
      }
      if (root.type === "canvas_arc") {
        addBlock("canvas", "arc", root);
        continue;
      }
      if (root.type === "canvas_rect") {
        addBlock("canvas", "rect", root);
        continue;
      }
      if (root.type === "canvas_clip") {
        addBlock("canvas", "clip", root);
        continue;
      }
      if (root.type === "canvas_setLineWidth") {
        addBlock("canvas", "setLineWidth", root);
        continue;
      }
      if (root.type === "canvas_setLineCap") {
        addBlock("canvas", "setLineCap", root);
        continue;
      }
      if (root.type === "canvas_setStrokeStyle") {
        addBlock("canvas", "setStrokeStyle", root);
        continue;
      }
      if (root.type === "canvas_setFillStyle") {
        addBlock("canvas", "setFillStyle", root);
        continue;
      }
      if (root.type === "canvas_stroke") {
        addBlock("canvas", "stroke", root);
        continue;
      }
      if (root.type === "canvas_fill") {
        addBlock("canvas", "fill", root);
        continue;
      }
      if (root.type === "canvas_clearRect") {
        addBlock("canvas", "clearRect", root);
        continue;
      }
      if (root.type === "canvas_setFont") {
        addBlock("canvas", "setFont", root);
        continue;
      }
      if (root.type === "canvas_strokeText") {
        addBlock("canvas", "strokeText", root);
        continue;
      }
      if (root.type === "canvas_fillText") {
        addBlock("canvas", "fillText", root);
        continue;
      }
      if (root.type === "canvas_measureText") {
        addBlock("canvas", "(measureText)", root);
        continue;
      }
      if (root.type === "canvas_loadImage") {
        addBlock("canvas", "loadImage", root);
        continue;
      }
      if (root.type === "canvas_drawImage") {
        addBlock("canvas", "drawImage", root);
        continue;
      }
      if (root.type === "canvas_scale") {
        addBlock("canvas", "scale", root);
        continue;
      }
      if (root.type === "canvas_rotate") {
        addBlock("canvas", "rotate", root);
        continue;
      }
      if (root.type === "canvas_translate") {
        addBlock("canvas", "translate", root);
        continue;
      }
      if (root.type === "canvas_transform") {
        addBlock("canvas", "transform", root);
        continue;
      }
      if (root.type === "canvas_clearTransform") {
        addBlock("canvas", "clearTransform", root);
        continue;
      }
      if (root.type === "canvas_save") {
        addBlock("canvas", "save", root);
        continue;
      }
      if (root.type === "canvas_restroke") {
        addBlock("canvas", "restroke", root);
        continue;
      }
      if (root.type === "canvas_setGlobalAlpha") {
        addBlock("canvas", "setGlobalAlpha", root);
        continue;
      }
      if (root.type === "canvas_setGlobalCompositeOperation") {
        addBlock("canvas", "setGlobalCompositeOperation", root);
        continue;
      }
      if (root.type === "canvas_switchCanvas") {
        addBlock("canvas", "switchCanvas", root);
        continue;
      }
      if (root.type === "canvas_stampOnStage") {
        addBlock("canvas", "stampOnStage", root);
        continue;
      }
      ...
      if (root.type === "lazyAudio_load") {
        addBlock("lazyaudio", "load()", root);
        continue;
      }
      if (root.type === "lazyAudio_playAndWait") {
        addBlock("lazyaudio", "playAndWait()", root);
        continue;
      }
      ...
      if (root.type === "community_isFollower") {
        addBlock("community", "<is follower?>", root);
        continue;
      }
      if (root.type === "community_isProjectLover") {
        addBlock("community", "<love this project?>", root);
        continue;
      }
      if (root.type === "community_openUrl") {
        addBlock("community", "open", root);
        continue;
      }
      if (root.type === "community_redirectUrl") {
        addBlock("community", "redirect", root);
        continue;
      }
      if (root.type === "community_pay") {
        addBlock("community", "pay()for()", root);
        continue;
      }
      if (root.type === "community_getError") {
        addBlock("community", "(error)", root);
        continue;
      }
    ...
    for (const row of vars) {
      addBlock(
        row.isLocal ? "var" : "VAR",
        (row.isLocal ? "私有变量 " : "公有变量 ") + row.name,
        row
      );
    }
    ...
    for (const row of lists) {
      addBlock(
        ...
        (row.isLocal ? "私有列表 " : "公有列表 ") + row.name,
        row
      );
    }
    const clsOrder = {
  ...
  function buildNavigationCarosel(nav, li, blocks) {
    if (nav && nav.parentNode === li) {
      ...
      li.insertAdjacentHTML(
        "beforeend",
        `<span id="s3devMulti"class="s3devMulti"><span id="s3devMultiLeft"class="s3devNav">◀</span><span id="s3devMultiCount"></span><span id="s3devMultiRight"class="s3devNav">▶</span></span>`
      );
      document
        .getElementById("s3devMultiLeft")
        .addEventListener("click", multi.navLeft);
      document
        .getElementById("s3devMultiRight")
        .addEventListener("click", multi.navRight);
      multi.idx = 0;
      multi.blocks = blocks;
      multi.update();
      if (blocks.length > 0) {
  ...
  let multi = {
    ...
    navSideways: function (e, dir) {
      if (multi.blocks && multi.blocks.length > 0) {
        multi.idx =
          (multi.idx + dir + multi.blocks.length) % multi.blocks.length;
        multi.update();
  ...
  function initInner() {
    ...
    root.insertAdjacentHTML(
      "beforeend",
      `<div id="s3devToolBar"><label class='title'id=s3devFindLabel><span>搜寻</span><span id=s3devFind><div id='s3devDDOut'><input id='s3devInp'type='search'placeholder='搜寻 (Ctrl+F)'autocomplete='off'><ul id='s3devDD'></ul></div></span><a id="s3devDeep"href="#">Deep</a><a id="s3devCleanUp"href="#">整理积木</a></label></div>`
    );
